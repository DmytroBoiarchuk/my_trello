...
export const getBoards = async (dispatch: Dispatch): Promise<void> => {
  try {
    ...
    const { boards }: Response = await axiosConfig.get(`${api.baseURL}/board`, config);
...
export const createBoard = async (title: string, dispatch: Dispatch): Promise<void> => {
  try {
    await axiosConfig.post(`${api.baseURL}/board`, { title });
...
export const deleteBoard =
  ...
  async (dispatch: Dispatch): Promise<void> => {
    try {
      await axiosConfig.delete(`${api.baseURL}/board/${id}`);
      await getBoards(dispatch);
    } catch (e) {
      dispatch({ type: 'ERROR_ACTION_TYPE' });
    }
